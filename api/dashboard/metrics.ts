...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const overdueInvoices = await sql`
      SELECT COUNT(*) as count, COALESCE(SUM(balance), 0) as amount
      FROM transactions WHERE type = 'invoice' AND status IN ('open', 'partial') AND due_date < NOW()
    `;
    ...
    return res.status(200).json({
      ...
      invoices: {
        ...
        overdue: { count: Number(overdueInvoices[0]?.count) || 0, amount: Number(overdueInvoices[0]?.amount) || 0 },
